...
const PaginationInUserDashBoard = () => {
    ...
    const totalPage = Math.ceil(totalUser / 8)
    return (
        <div className='pt-3'>
            <div className=' flex items-center max-sm:flex-col gap-3 justify-between'>
                ...
                <div className='flex items-center gap-3'>
                    ...
                    {[1, 2, 3].map((page) => (
                        <button key={page} disabled={page > totalPage} onClick={() => { setAdminPage(page) }} className={`border rounded-lg px-2.5 py-0.5 transition-all duration-200
      ${adminPage === page
                                ? "border-green-600"
                                : "border-zinc-600"
                            }
