...
import useUserPlaylistDashBoardData from '../../Hooks/useUserPlaylistDashBoardData'
import AllPlaylistInPlaylistDash from '../../component/adminPageComponent/playlistPagecomponent/AllPlaylistInPlaylistDash'

const AdminPlaylistpage = () => {
  const playlistData = useUserPlaylistDashBoardData()
  return (
    <div className=' lg:w-[75%] w-[67%] md:w-[67%] max-sm:ml-0 md:ml-[250px] lg:ml-[250px] xl:ml-[320px]  max-sm:w-full  border-t mt-5 border-[#2e2e2e] p-6  bg-[#121212]'>
      <h1 className='text-xl font-semibold leading-5'>
        Playlists <br />
        <span className='text-[12px] font-normal text-gray-400'>Manage and oversee platform playlists</span>
      </h1>
      <div className=' pt-2'>
        <div className='flex items-center justify-center flex-wrap gap-2 flex-warp shrink-0'>
          {
            playlistData.map((elem) => {
              ...
              return <div className='lg:w-58 max-sm:w-60 md:w-50 w-50 h-32  bg-[#141414] border border-zinc-700 rounded-xl  hover:-translate-y-1.5
