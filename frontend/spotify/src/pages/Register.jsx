...
function Register() {
  ...
  return (
    <div className='text-white'>
      {
        user ? (<Home />) : (<div className='flex items-center justify-center px-4 pt-0 lg:pt-3 md:pt-0 max-sm:pt-0'>
          <div className='w-full max-w-md flex justify-center md:pt-4 pt-4 lg:pt-3 max-sm:pt-5'>
            <div className='text-center w-100 max-sm:w-70'>
              ...
              <form onSubmit={handleSumbit} className='text-start md:pt-0 pt-0 lg:pt-5 max-sm:pt-0'>
                ...
                <input required type="name" value={username} className={`w-full py-2.5 max-sm:py-1.5 max-sm:text-sm  mb-4 max-sm:mb-2 px-3 border ${alreadyExist ? "border-red-500" : "border-[#7C7C7C]"}  rounded`} onChange={(elem) => { setUsername(elem.target.value) }} placeholder='Enter name' />
                ...
                <button
                  ...
                  ${regValid && !loading ? 'bg-[#2beb6e] text-black cursor-pointer opacity-100' : 'cursor-not-allowed bg-[#2beb6e] text-black opacity-75'}
