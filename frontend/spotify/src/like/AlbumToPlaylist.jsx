...
const AlbumToPlaylist = ({elem}) => {
    let {getPlayList,hideAlbumPlaylist,setHideAlbumPlaylist}=useContext(authProvider)
  ...
  <div>
        ...
        hideAlbumPlaylist && (
          <>
            <div onClick={() => { setHideAlbumPlaylist(false) }} className='w-full cursor-pointer h-full inset-0 bg-black/50 backdrop:backdrop-blur-sm  absolute z-14 '></div>
              ...
              <div className='w-72 max-h-80 overflow-hidden rounded-2xl border border-white/10 bg-[#1e1e1e]/95 backdrop-blur-xl shadow-2xl animate-in fade-in zoom-in-95 duration-200'>
                ...
                <div className='sticky top-0 z-10 border-b border-white/10 bg-[#252525] px-4 py-3'>
                  <div className='flex items-center justify-between'>
                    ...
                    <h1 onClick={() => { setHideAlbumPlaylist(false) }}>
                      X
                    </h1>
                  </div>
                ...
                <div className='max-h-64 overflow-y-auto p-2 custom-scroll'>
                  ...
                  {getPlayList.length > 0 ? (
                    getPlayList?.map((dataId, index) => {
                      const separateId = dataId?.music?.some((song) =>
                        song._id === elem._id
                      )
                        ...
                        <>
                          {
                            separateId ? (
                              <div  onClick={() => {
                                deleteApi(dataId._id, elem._id)
                                setHideAlbumPlaylist(false)
                              }} key={dataId._id} className='group flex items-center gap-3 rounded-xl px-3 py-3 transition-all duration-200 hover:bg-white/10 hover:scale-[1.02] cursor-pointer'>
...
export default AlbumToPlaylist
